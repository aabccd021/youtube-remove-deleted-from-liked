...
fs.readFile('client_secret.json', function processClientSecrets(err, content) {
  ...
  authorize(JSON.parse(content), listLikedAndRemoveDeletedVideos);
});
...
async function listLikedAndRemoveDeletedVideos(auth) {
  var service = google.youtube('v3');
...
async function _listLiked(auth) {
  var service = google.youtube('v3');
  var pageToken = undefined;
  var titles = [];
  for (i = 0; i < 6; i++) {
    ...
    const pageTitles = response.data.items.map((item) => item.snippet.title);
    titles = [...titles, ...pageTitles]
  }
  fs.writeFileSync('a.json', JSON.stringify(titles, undefined, 2))
}
